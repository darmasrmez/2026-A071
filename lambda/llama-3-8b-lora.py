...
pkgs = ['python-dotenv', 'trackio', 'transformers', 'trl[peft]', 'accelerate', 'bitsandbytes', 'datasets', 'ninja', 'codecarbon', 'packaging', 'zeus', 'wandb', 'huggingface-hub', 'tqdm', 'evaluate', 'nltk', 'pandas', 'matplotlib', 'prometheus-client', 'kernels']
...
os.makedirs('code_carbon_llama-3-8b-lora', exist_ok=True)


if is_main:
    log_name = "bio-llama-3-8b-lora_logs"
    ...
    POWER_CSV = './code_carbon_llama-3-8b-lora/power_timeseries.csv'
    with open(POWER_CSV, 'w') as _f:
        _f.write('timestamp,cpu_w,ram_w,cpu_e,ram_e,gpu_e,gpus,phase\n')
    PHASE_ENERGY_CSV = './code_carbon_llama-3-8b-lora/phase_energy.csv'
    ...
    tracker = EmissionsTracker(
        project_name = 'bio-llama-3-8b-lora',
        output_dir="./code_carbon_llama-3-8b-lora/",
        save_to_file=True,
        on_csv_write='append',
...
training_arguments = TrainingArguments(
    ...
    push_to_hub=True,
    hub_token=hf_token,
    hub_model_id="darmasrmz/bio-llama-3-8b-lora",
    hub_strategy="end",
)
...
if is_main:
    tracker.stop()
    new_model = 'bio-llama-3-8b-lora'
    model.save_pretrained(new_model)
    model.push_to_hub("darmasrmz/bio-llama-3-8b-lora")
    tokenizer.save_pretrained(new_model)
    tokenizer.push_to_hub("darmasrmz/bio-llama-3-8b-lora")
...
if is_main:
    ...
    energy_summary.to_csv('bio-llama-3-8b-lora-energy_summary.csv', index=False)