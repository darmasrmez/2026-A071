...
pkgs = ['python-dotenv', 'trackio', 'transformers', 'trl[peft]', 'accelerate', 'bitsandbytes', 'datasets', 'ninja', 'codecarbon', 'packaging', 'zeus', 'wandb', 'huggingface-hub', 'tqdm', 'evaluate', 'nltk', 'pandas', 'matplotlib', 'prometheus-client']
...
os.makedirs('code_carbon_llama-3-8b-qlora', exist_ok=True)


if is_main:
    log_name = "bio-llama-3-8b-qlora_logs"
    ...
    POWER_CSV = './code_carbon_llama-3-8b-qlora/power_timeseries.csv'
    with open(POWER_CSV, 'w') as _f:
        _f.write('timestamp,cpu_w,ram_w,cpu_e,ram_e,gpu_e,gpus,phase\n')
    PHASE_ENERGY_CSV = './code_carbon_llama-3-8b-qlora/phase_energy.csv'
    ...
    tracker = EmissionsTracker(
        project_name = 'bio-llama-3-8b-qlora',
        output_dir="./code_carbon_llama-3-8b-qlora/",
        save_to_file=True,
        on_csv_write='append',
...
dataset = dataset.map(format_chat_template, remove_columns=[], batched=False)
...
training_arguments = TrainingArguments(
    ...
    push_to_hub=True,
    hub_token=hf_token,
    hub_model_id="darmasrmz/bio-llama-3-8b-qlora",
    hub_strategy="end",
)
...
if is_main:
    tracker.stop()
    new_model = 'bio-llama-3-8b-qlora'
    model.save_pretrained(new_model)
    model.push_to_hub("darmasrmz/bio-llama-3-8b-qlora")
    tokenizer.save_pretrained(new_model)
    tokenizer.push_to_hub("darmasrmz/bio-llama-3-8b-qlora")
...
if is_main:
    ...
    energy_summary.to_csv('bio-llama-3-8b-qlora-energy_summary.csv', index=False)