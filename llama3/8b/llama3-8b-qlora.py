...
if is_main:
    os.makedirs('code_carbon_bio-llama3-8b-qlora', exist_ok=True)

    log_name = "bio-llama3-8b-qlora-logs"
    ...
    POWER_CSV = './code_carbon_bio-llama3-8b-qlora/power_timeseries.csv'
    ...
    tracker = EmissionsTracker(
        project_name='bio-llama3-8b-qlora',
        output_dir="./code_carbon_bio-llama3-8b-qlora/",
        save_to_file=True,
        on_csv_write='append',
...
print('Downloading model in 4-bit (QLoRA)')
...
if is_main:
    new_model = 'Bio-llama3-8b-qlora'
    trainer.model.save_pretrained(new_model)
    trainer.model.push_to_hub("darmasrmz/bio-llama3-8b-qlora", token=hf_token)
    tokenizer.push_to_hub("darmasrmz/bio-llama3-8b-qlora", token=hf_token)
...
"""
    ...
    plt.title('Función de pérdida (QLoRA)')
    plt.xlabel('Época')
    plt.ylabel('Pérdida')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('loss_function_qlora.png', transparent=True)
    plt.close()
    ...
    try:
        ...
        if not power_df.empty:
            ...
            ax.set_title('Consumo de potencia (CPU, RAM, GPU) — QLoRA')
            ax.set_xlabel('Tiempo')
            ax.set_ylabel('Potencia (W)')
            ax.legend(loc='upper right')
            ax.grid(True)
            fig.tight_layout()
            fig.savefig('power_consumption_qlora.png', transparent=True)
            plt.close(fig)
